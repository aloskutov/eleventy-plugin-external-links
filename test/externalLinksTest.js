...
test('local links with query string', (t) => {
  const outputPath = 'test.html';
  const content = '<!doctype html><html><head></head><body>' +
  '<p>Lorem ipsum dolor sit <a href="/what-is/?amet">amet</a>, consectetur adipiscing elit. Phasellus ac nunc ut enim lobortis dignissim. Praesent nisl dui, venenatis a tortor sit amet, bibendum vestibulum mauris. Sed sed pulvinar nunc. Morbi quis efficitur ipsum. <a href="?another-link=123">Proin vestibulum eleifend</a> sem eu sodales. Phasellus id est vel eros commodo placerat eu consectetur arcu. Vestibulum egestas mauris non aliquet varius.</p>' +
  '</body></html>';
  t.is(externalLinks(content, outputPath), content);
});

test('local links with id', (t) => {
  const outputPath = 'test.html';
  const content = '<!doctype html><html><head></head><body>' +
  '<p>Lorem ipsum dolor sit <a href="#amet">amet</a>, consectetur adipiscing elit. Phasellus ac nunc ut enim lobortis dignissim. Praesent nisl dui, venenatis a tortor sit amet, bibendum vestibulum mauris. Sed sed pulvinar nunc. Morbi quis efficitur ipsum. <a href="/another-link/#123">Proin vestibulum eleifend</a> sem eu sodales. Phasellus id est vel eros commodo placerat eu consectetur arcu. Vestibulum egestas mauris non aliquet varius.</p>' +
  '</body></html>';
  t.is(externalLinks(content, outputPath), content);
});

test('Local links, with domain', (t) => {
  const outputPath = 'test.html';
